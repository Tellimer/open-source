...
console.log(`  Within-Year Increases: ${result.evidence.within_year_increases}`);
...
if (result.evidence.reset_points && result.evidence.reset_points.length > 0) {
  ...
  result.evidence.reset_points.forEach((point, idx) => {
    console.log(`    ${idx + 1}. ${point.from_date} → ${point.to_date}: dropped ${point.dropped_by}%`);
  });
}

if (result.evidence.increase_points && result.evidence.increase_points.length > 0) {
  console.log('\n  Increase Points (first 3):');
  result.evidence.increase_points.forEach((point, idx) => {
    console.log(`    ${idx + 1}. ${point.from_date} → ${point.to_date}: increased ${point.increased_by}%`);
  });
}
...
if (result.is_cumulative && result.pattern_type === 'ytd' && result.confidence > 0.8) {
  console.log('✅ TEST PASSED: Correctly detected YTD cumulative pattern!');
} else {
  console.log('❌ TEST FAILED: Expected YTD cumulative pattern');
  console.log(`   Got: is_cumulative=${result.is_cumulative}, pattern_type=${result.pattern_type}, confidence=${result.confidence}`);
}
