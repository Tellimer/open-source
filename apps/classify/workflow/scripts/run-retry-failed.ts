...
/**
 * Retry classification for indicators that failed
 *
 * Usage:
 *   deno task run:retry-failed              # Retry all failed with OpenAI
 *   deno task run:retry-failed openai       # Retry all failed with OpenAI
 *   deno task run:retry-failed local        # Retry all failed with local LLM
 *   deno task run:retry-failed anthropic    # Retry all failed with Anthropic
 */
...
/**
 * Get indicators that have failed at any stage
 */
function getFailedIndicators(db: Database): SourceIndicator[] {
  const query = `
    SELECT DISTINCT
      s.id,
      s.name,
...
async function classifyIndicatorsBatch(
  ...
  console.log(
    `\n🔄 Retrying ${totalIndicators} failed indicators in ${numBatches} batches of ${batchSize}...`
  );
...
async function main() {
  ...
  try {
    ...
    const failedStmt = db.prepare(
      'SELECT COUNT(DISTINCT indicator_id) FROM processing_log WHERE status = "failed"'
    );
    const failedCount = failedStmt.value()![0] as number;

    console.log(`📊 Total indicators: ${totalCount.toLocaleString()}`);
    console.log(`❌ Failed indicators: ${failedCount.toLocaleString()}`);

    if (failedCount === 0) {
      console.log('\n✅ No failed indicators to retry! All processing succeeded.');
      return;
    }

    // Get failed indicators
    console.log(`\n🎯 Fetching failed indicators...`);
    const indicators = getFailedIndicators(db);

    console.log(`✅ Found ${indicators.length} failed indicators\n`);

    // Show first few
    console.log('First 5 failed indicators:');
