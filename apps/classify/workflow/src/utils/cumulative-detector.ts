...
export interface CumulativeDetectionResult {
  ...
  evidence: {
    ...
    reset_points?: Array<{ from_date: string; to_date: string; dropped_by: number }>;
    increase_points?: Array<{ from_date: string; to_date: string; increased_by: number }>;
  };
  reasoning: string;
}
 ...
 */
function parseDateToComponents(dateStr: string): { year: number; month: number; quarter: number } | null {
  try {
 ...
 */
function isDifferentYear(date1Components: ReturnType<typeof parseDateToComponents>,
                         date2Components: ReturnType<typeof parseDateToComponents>): boolean {
 ...
 */
function isLaterInSameYear(date1Components: ReturnType<typeof parseDateToComponents>,
                           date2Components: ReturnType<typeof parseDateToComponents>): boolean {
  if (!date1Components || !date2Components) return false;
  return date1Components.year === date2Components.year &&
...
export function detectCumulativePattern(
  ...
  if (!sampleValues || sampleValues.length < 3) {
    return {
      ...
      reasoning: 'Insufficient data points for cumulative pattern detection (need at least 3 points)',
    };
  }
  ...
  const resetPoints: Array<{ from_date: string; to_date: string; dropped_by: number }> = [];
  const increasePoints: Array<{ from_date: string; to_date: string; increased_by: number }> = [];
  ...
  if (isYTD) {
    ...
    const ytdConsistency = (yearResets + withinYearIncreases) / totalTransitions;
    confidence = Math.min(0.95, ytdConsistency);

    reasoning = `Detected YTD (Year-to-Date) cumulative pattern: ${yearResets} year ${yearResets === 1 ? 'reset' : 'resets'} observed where values dropped significantly at year boundaries, and ${withinYearIncreases} within-year increases where values accumulated throughout the year. This indicates the data represents cumulative totals that reset annually.`;
  } else if (isRunningTotal) {
    patternType = 'running_total';
    confidence = Math.min(0.90, withinYearIncreases / totalTransitions);

    reasoning = `Detected running total cumulative pattern: ${withinYearIncreases} consecutive increases with no resets observed. Values continuously accumulate without year-boundary resets.`;
  } else {
    ...
    if (withinYearDecreases > 0) {
      reasoning += `Found ${withinYearDecreases} within-year decreases, indicating period-specific values rather than accumulation. `;
    }
    if (yearResets === 0 && withinYearTransitions > 0) {
      reasoning += `No year-boundary resets observed across ${totalTransitions} transitions. `;
    }
    reasoning += `Data appears to represent discrete period totals or averages.`;
  }
  ...
  return {
    ...
    evidence: {
      ...
      reset_points: resetPoints.length > 0 ? resetPoints.slice(0, 3) : undefined, // Show first 3
      increase_points: increasePoints.length > 0 ? increasePoints.slice(0, 3) : undefined,
    },
    reasoning,
  };
}
