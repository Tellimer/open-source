/**
 * Run classification on sampled indicators using OpenAI
 *
 * Usage:
 *   deno task run:openai -100   # Run 100 indicators with OpenAI GPT-4o
 *   deno task run:openai -25    # Run 25 indicators (estimated $0.19)
 ...
 */
function parseArgs(): { sampleSize: number } {
...
async function classifyIndicators(
  ...
  const costPerIndicator = 0.00763; // From COST_ESTIMATE.md
  const estimatedCost = (indicators.length * costPerIndicator).toFixed(2);

  console.log(`🚀 Starting OpenAI classification...`);
  console.log(`   API: ${apiUrl}`);
  console.log(`   LLM Provider: openai (gpt-4.1-mini)`);
  console.log(`   Total indicators: ${indicators.length}`);
  console.log(`   API batches: ${batches} (max ${API_BATCH_SIZE} per request)`);
  console.log(`   Internal batch size: 5 indicators (OOM prevention)`);
...
async function main() {
  ...
  const results = await classifyIndicators(indicators, apiUrl);
