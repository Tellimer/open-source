...
const DEFAULT_TABLES = [
  'source_indicators', 'classifications', 'normalization_results',
  'time_inference_results', 'cumulative_detection_results',
  'scale_inference_results', 'currency_check_results',
  'family_assignment_results', 'type_classification_results',
  'boolean_review_results', 'final_review_results',
  'processing_log', 'pipeline_stats',
];

function parseArgs() {
  const options = { batchSize: 100, dryRun: false, tables: undefined as string[] | undefined };
  for (const arg of Deno.args) {
    if (arg.startsWith('--tables=')) options.tables = arg.slice(9).split(',');
    else if (arg.startsWith('--batch-size=')) options.batchSize = parseInt(arg.slice(13));
...
function getPrimaryKey(table: string): string {
  const keys: Record<string, string> = {
    source_indicators: 'id', processing_log: 'id', pipeline_stats: 'id',
...
function convertValue(value: any, columnType: string): any {
  ...
  if (columnType.includes('BOOLEAN') && typeof value === 'number') return value === 1;
  return value;
}
...
async function syncTable(sqliteDb: Database.Database, sql: postgres.Sql, table: string, options: any) {
  console.log(`\n📊 Syncing: ${table}`);
  ...
  for (let i = 0; i < rows.length; i += options.batchSize) {
    ...
    for (const row of batch) {
      try {
        const values = columns.map(col => convertValue(row[col], getColumnType(table, col)));
        const placeholders = columns.map((_, idx) => `$${idx + 1}`).join(', ');
        const updateSet = columns.filter(c => c !== primaryKey).map(c => `${c} = EXCLUDED.${c}`).join(', ');
    ...
    Deno.stdout.writeSync(new TextEncoder().encode(`\r  ⏳ ${progress}/${rows.length}`));
  }
...
async function main() {
  ...
  const sqlitePath = Deno.env.get('CLASSIFY_DB_LOCAL_DEV') || './data/classify-workflow-local-dev.db';

  console.log(`📂 SQLite: ${sqlitePath}`);
  console.log(`🗄️  PostgreSQL: ${postgresUrl.replace(/:[^:@]+@/, ':****@')}`);
  console.log(`📋 Tables: ${options.tables?.join(', ') || `All (${DEFAULT_TABLES.length})`}`);
  ...
  try {
    if (!options.dryRun) {
      console.log('\n📝 Ensuring schema...');
      const statements = CLASSIFY_WORKFLOW_POSTGRES_SCHEMA.split(';').filter(s => s.trim());
    ...
    for (const table of tables) {
      ...
      const { synced, skipped } = await syncTable(sqliteDb, sql, table, options);
      totalSynced += synced;
