...
console.log(
  `✅ Processed ${result1.data.length} indicators with explicit metadata\n`,
);
...
result1.data.forEach((item) => {
  ...
  if (item.explain) {
    ...
    if (item.explain.periodicity) {
      ...
      console.log(
        `   ⏰ Time: ${p.original || "none"} → ${p.target} (adjusted: ${
          p.adjusted ? "Yes" : "No"
        })`,
      );
    }
    ...
    if (item.explain.fx) {
      console.log(
        `   💱 FX: ${item.explain.fx.rate} ${item.explain.fx.currency}/USD`,
      );
    }
    ...
    if (item.explain.magnitude) {
      ...
      console.log(
        `   📏 Scale: ${m.originalScale} → ${m.targetScale} (factor: ${m.factor}x)`,
      );
    }
  }
  console.log();
...
console.log(
  `✅ Processed ${result2.data.length} indicators with mixed metadata\n`,
);
...
result2.data.forEach((item) => {
  ...
  if (item.explain) {
    ...
    if (item.explain.fx) {
      console.log(
        `      💱 Currency: ${item.explain.fx.currency} (from ${
          item.currency_code ? "explicit field" : "unit string"
        })`,
      );
    }
    ...
    if (item.explain.magnitude) {
      console.log(
        `      📏 Scale: ${item.explain.magnitude.originalScale} (from ${
          item.scale ? "explicit field" : "unit string"
        })`,
      );
    }
    ...
    if (item.explain.periodicity) {
      console.log(
        `      ⏰ Periodicity: ${item.explain.periodicity.original} (from ${
          item.periodicity ? "explicit field" : "unit string"
        })`,
      );
    }
  }
...
console.log(
  `✅ Processed ${result3.data.length} indicator with fallback parsing\n`,
);
...
result3.data.forEach((item) => {
  ...
  console.log(
    `   🔍 All metadata parsed from unit string (explicit fields were null/empty)`,
  );

  if (item.explain?.periodicity) {
    console.log(
      `   ⏰ Time conversion: ${item.explain.periodicity.original} → ${item.explain.periodicity.target}`,
    );
  }
  console.log();
});
