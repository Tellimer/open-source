...
export const monetaryNormalizationMachine = setup({
  ...
  actors: {
    resolveAutoTargets: fromPromise(async ({ input }: { input: MonetaryNormalizationInput }) => {
      if (input.autoTargets) return input.autoTargets;
      if (!input.config.autoTargetByIndicator) return undefined as unknown as AutoTargets;
    ...
    normalizeGroup: fromPromise(async ({ input }: { input: MonetaryNormalizationInput & { group: Group } }) => {
      ...
      const res = await processBatch(input.group.items, {
        ...
        toMagnitude: (sel?.magnitude as Scale | undefined) ?? (config.targetMagnitude as Scale | undefined),
        toTimeScale: (sel?.time as TimeScale | undefined) ?? preferredTimeScale ?? config.targetTimeScale,
        fx,
          ...
          if (sel) {
            (m.explain ||= {}).targetSelection = {
              ...
              selected: {
                ...
                time: (sel.time as TimeScale | undefined) ?? preferredTimeScale ?? (config.targetTimeScale as TimeScale | undefined),
              },
    ...
    batchAll: fromPromise(async ({ input }: { input: MonetaryNormalizationInput }) => {
      const { config, items, fx, fxSource, fxSourceId, preferredTimeScale } = input;
  ...
  states: {
    prepareBatchOptions: {
      always: [
        { guard: ({ context }) => !!context.config.autoTargetByIndicator, target: "resolveAutoTargets" },
        { target: "batchAll" },
      ],
    },
    resolveAutoTargets: {
      invoke: {
        ...
        onDone: {
          ...
          actions: assign({ autoTargets: ({ event }) => (event as any).output }),
        },
    ...
    groupByIndicator: {
      entry: assign(({ context }) => {
        ...
        const keyField = (context.config.indicatorKey ?? "name") as keyof ParsedData;
        ...
        for (const [key, items] of groupsMap.entries()) {
          const selection = context.autoTargets?.get(key) as AutoTargetSelection | undefined;
    ...
    normalizeNextGroup: {
      always: [
        { guard: ({ context }) => (context.groups?.length ?? 0) === 0, target: "enrichExplainFX" },
        { guard: ({ context }) => (context.groupIndex ?? 0) >= (context.groups?.length ?? 0), target: "enrichExplainFX" },
        { target: "runGroup" },
      ],
    },
    runGroup: {
    ...
    accumulateAndNext: {
      entry: assign(({ context }) => ({ groupIndex: (context.groupIndex ?? 0) + 1 })),
      always: { target: "normalizeNextGroup" },
    },
    batchAll: {
      invoke: {
        ...
        onDone: {
          ...
          actions: assign({ out: ({ event }) => (event as any).output as ParsedData[] }),
        },
      },
    },
    ...
    enrichExplainFX: {
      ...
      type: "final",
      output: ({ context }): MonetaryNormalizationOutput => ({ items: context.out }),
    },
  },
  output: ({ context }): MonetaryNormalizationOutput => ({ items: context.out }),
});
