...
export const CURRENCY_CODES = new Set([
  ...
  "NZD", "SGD", "HKD", "KRW", "INR", "RUB", "BRL", "MXN", "ZAR", "TRY", "AED",
  "SAR", "THB", "MYR", "IDR", "PHP", "VND", "PKR", "BDT", "NGN", "EGP", "ARS",
  "COP", "CLP", "PEN", "UYU", "BOB", "PYG", "VES", "CRC", "GTQ", "HNL", "NIO",
  "DOP", "JMD", "TTD", "BBD", "BSD", "BZD", "XCD", "HTG", "SRD", "GYD", "AWG",
  "KYD", "BMD", "PAB", "MAD", "DZD", "TND", "LYD", "JOD", "LBP", "SYP", "IQD",
  "KWD", "BHD", "OMR", "QAR", "ILS", "GEL", "AMD", "AZN", "KZT", "KGS", "TJS",
  "TMT", "UZS", "AFN", "MMK", "LAK", "KHR", "NPR", "LKR", "MVR", "BTN", "MNT",
  "KPW", "TWD", "MOP", "BND", "FJD", "PGK", "WST", "SBD", "TOP", "VUV", "XPF",
  "ETB", "KES", "TZS", "UGX", "RWF", "BIF", "MGA", "MWK", "MZN", "ZMW", "ZWL",
  "BWP", "NAD", "SZL", "LSL", "GHS", "GMD", "GNF", "LRD", "SLL", "XOF", "XAF",
  "CVE", "STN", "SCR", "MUR", "KMF", "DJF", "ERN", "SSP", "SDG", "MRU", "ALL",
  "MKD", "RSD", "BAM", "BGN", "RON", "MDL", "UAH", "BYN", "PLN", "CZK", "HUF",
  "HRK", "ISK", "CUP", "AOA", "CDF"
]);
...
export const MAGNITUDE_PATTERNS: UnitPattern[] = [
  ...
  { pattern: /\bbill?i?on?s?\b|\bbn\b/i, category: "currency", scale: "billions" },
  { pattern: /\bmill?i?on?s?\b|\bmn\b|\bmio\b/i, category: "currency", scale: "millions" },
  { pattern: /\bthou?sand?s?\b|\bk\b/i, category: "currency", scale: "thousands" },
...
export const TIME_PATTERNS: UnitPattern[] = [
  { pattern: /per\s+year|\/year|\/yr|annual/i, category: "time", timeScale: "year" },
  { pattern: /per\s+quarter|\/quarter|\/q\b|quarterly/i, category: "time", timeScale: "quarter" },
  { pattern: /per\s+month|\/month|\/mo\b|monthly/i, category: "time", timeScale: "month" },
  { pattern: /per\s+week|\/week|\/wk\b|weekly/i, category: "time", timeScale: "week" },
  { pattern: /per\s+day|\/day|\/d\b|daily/i, category: "time", timeScale: "day" },
  { pattern: /per\s+hour|\/hour|\/hr?\b|hourly/i, category: "time", timeScale: "hour" }
];

// Percentage and ratio patterns
export const PERCENTAGE_PATTERNS: UnitPattern[] = [
  { pattern: /percent\s+of\s+GDP/i, category: "percentage", normalized: "% of GDP" },
  { pattern: /percent\s+of\s+potential\s+GDP/i, category: "percentage", normalized: "% of potential GDP" },
  { pattern: /percent\s+change/i, category: "percentage", normalized: "% change" },
  { pattern: /percentage\s+points?/i, category: "percentage", normalized: "pp" },
  { pattern: /percent|%/i, category: "percentage", normalized: "%" },
  { pattern: /basis\s+points?|bps/i, category: "percentage", normalized: "bps" }
];

// Physical quantity patterns
export const PHYSICAL_PATTERNS: UnitPattern[] = [
  { pattern: /BBL\/D\/1K|barrels?\s+per\s+day/i, category: "physical", normalized: "BBL/D/1K" },
  { pattern: /thousand\s+tonnes/i, category: "physical", normalized: "thousand tonnes" },
  ...
  { pattern: /liters?|litres?|liter\b|l\b/i, category: "physical", normalized: "liters" },
  { pattern: /gallons?/i, category: "physical", normalized: "gallons" },
  { pattern: /cubic\s+feet/i, category: "physical", normalized: "cubic feet" },
  { pattern: /square\s+met(?:re|er)/i, category: "physical", normalized: "sq meter" },
...
export const ENERGY_PATTERNS: UnitPattern[] = [
  { pattern: /gigawatt[\s-]?hours?|gwh/i, category: "energy", normalized: "GWh" },
...
export const COUNT_PATTERNS: UnitPattern[] = [
  { pattern: /doses?\s+per\s+100\s+people/i, category: "count", normalized: "doses per 100 people" },
  { pattern: /per\s+1000\s+people/i, category: "rate", normalized: "per 1000 people" },
  { pattern: /per\s+(?:one\s+)?million\s+people/i, category: "rate", normalized: "per million people" },
  ...
  { pattern: /companies?(?:\s+and\s+individuals)?/i, category: "count", normalized: "companies" },
  { pattern: /individuals?/i, category: "count", normalized: "individuals" },
  { pattern: /households?/i, category: "count", normalized: "households" },
  { pattern: /workers?|employees?/i, category: "population", normalized: "workers" },
 ...
 */
function detectRatio(unitText: string): { numerator?: string; denominator?: string; isRatio: boolean } {
...
export function parseUnit(unitText: string): ParsedUnit {
  ...
  if (!normalized.includes("thousand tonnes") && !normalized.includes("thousand tons")) {
