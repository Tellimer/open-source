...
import { estimateTokens, calculateCost } from './token_counter.ts';
...
export function generateMockClassification(
  ...
  const id = indicator.id || `mock_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  // Detect monetary
  if (indicator.currency_code || units.includes('usd') || units.includes('billion') || units.includes('million')) {
    isMonetary = true;
  }

  // Detect type based on name/units
  if (name.includes('gdp') || name.includes('revenue') || name.includes('export') || name.includes('production')) {
    indicatorType = 'flow';
    indicatorCategory = 'physical-fundamental';
  } else if (name.includes('debt') || name.includes('reserve') || name.includes('stock')) {
    indicatorType = 'stock';
    indicatorCategory = 'physical-fundamental';
  } else if (name.includes('balance')) {
    indicatorType = 'balance';
    indicatorCategory = 'physical-fundamental';
  } else if (units.includes('%') || name.includes('rate') && !name.includes('exchange')) {
    if (name.includes('growth') || name.includes('change') || name.includes('inflation')) {
      indicatorType = 'rate';
  ...
  } else if (name.includes('index') || name.includes('s&p') || name.includes('cpi')) {
    indicatorType = 'index';
  ...
  } else if (name.includes('count') || name.includes('starts') || name.includes('number')) {
    indicatorType = 'count';
    indicatorCategory = 'numeric-measurement';
  }

  // Determine temporal aggregation
  let temporalAggregation: ClassifiedMetadata['temporal_aggregation'] = 'not-applicable';
  if (indicatorType === 'stock' || indicatorType === 'price' || indicatorType === 'index') {
  ...
  let heatMapOrientation: ClassifiedMetadata['heat_map_orientation'] = 'higher-is-positive';
  if (name.includes('unemployment') || name.includes('deficit') || name.includes('volatility')) {
  ...
  return {
    ...
    reasoning: '[DRY RUN] This is a simulated classification based on simple heuristics.',
  };
}
...
export function estimateDryRunCost(
  ...
  const { inputTokens, outputTokens } = estimateBatchTokens(indicators, includeReasoning);
  const totalTokens = inputTokens + outputTokens;
  const estimatedCost = calculateCost(inputTokens, outputTokens, provider, actualModel);
...
export async function printDryRunInfo(
  ...
  const estimate = estimateDryRunCost(indicators, provider, model, includeReasoning);
