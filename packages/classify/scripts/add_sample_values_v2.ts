...
for (const indicator of TEST_INDICATORS) {
  ...
  if (values.length > 0) {
    ...
    console.log(`  ✓ ${indicator.id}: ${sampleValues.length} values (${sampleValues[0].date} to ${sampleValues[sampleValues.length - 1].date})`);
    ...
    console.log(`  ⚠️  ${indicator.id}: No time series data found - using empty array`);
    indicatorsWithSamples.push({
      ...indicator,
      sample_values: [],
    });
...
export const TEST_INDICATORS: TestIndicatorFixture[] = ${JSON.stringify(indicatorsWithSamples, null, 2)};

export const TEST_INDICATOR_IDS = TEST_INDICATORS.map(ind => ind.id);
`;

await Deno.writeTextFile('./tests/fixtures/v2-test-indicators.ts', fixtureContent);

console.log(`\n✅ Sample values added to ./tests/fixtures/v2-test-indicators.ts`);
console.log(`   Total indicators: ${indicatorsWithSamples.length}`);
console.log(`   Indicators with data: ${indicatorsWithSamples.filter(i => i.sample_values.length > 0).length}\n`);
