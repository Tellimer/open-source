...
async function runReview() {
  ...
  try {
    ...
    const classifiedCount = db
      .prepare('SELECT COUNT(*) as count FROM classifications WHERE indicator_type IS NOT NULL')
      .value() as number[];
    ...
    console.log(`💰 Tokens Used: ${reviewResult.usage.totalTokens.toLocaleString()}`);
    console.log('='.repeat(60));
    ...
    const fixedDecisions = reviewResult.decisions.filter((d) => d.action === 'fix');
    ...
    if (escalatedDecisions.length > 0) {
      ...
      for (const decision of escalatedDecisions.slice(0, 5)) {
        ...
        if (decision.diff && Object.keys(decision.diff).length > 0) {
          console.log(`    Suggested changes: ${JSON.stringify(decision.diff)}`);
        }
      }
    ...
    if (escalatedDecisions.length > 0) {
      ...
      console.log('🎉 All indicators passed review - no human review needed!\n');
    }
