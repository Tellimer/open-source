...
import { applyFlaggingRules, DEFAULT_THRESHOLDS } from '../../src/v2/review/flagging.ts';
...
async function runFlagging() {
  ...
  try {
    ...
    console.log(`   Mode: Review ALL indicators (not just threshold violations)\n`);
    ...
    const flaggedIndicators = flaggingData.flatMap((data) => {
      ...
      const reviewAllFlag = {
        ...
        flag_reason: 'Production review - all indicators flagged for manual verification',
    ...
    for (const flag of flaggedIndicators) {
      flagsByType.set(flag.flag_type, (flagsByType.get(flag.flag_type) || 0) + 1);
    }
    ...
    console.log(`Total flags: ${flaggedIndicators.length}`);
    console.log(`Clean indicators: ${rows.length - uniqueFlaggedIndicators.size}\n`);
    console.log('Flags by type:');
    for (const [type, count] of Array.from(flagsByType.entries()).sort((a, b) => b[1] - a[1])) {
