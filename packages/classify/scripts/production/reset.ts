...
async function resetProductionDatabase() {
  ...
  console.log('⚠️  Source indicators and time series data will be preserved.\n');
  ...
  try {
    ...
    try {
      const indicators = await client.execute('SELECT COUNT(*) as count FROM source_indicators');
      const countryIndicators = await client.execute('SELECT COUNT(*) as count FROM source_country_indicators');
      console.log(`   • source_indicators:         ${indicators.rows[0]?.count || 0}`);
      console.log(`   • source_country_indicators: ${countryIndicators.rows[0]?.count || 0}`);
    } catch (error) {
    ...
    console.log('  Run: deno task prod:run      # Run classification pipeline\n');
