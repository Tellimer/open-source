...
async function main() {
  ...
  console.log(
    `Total estimated cost: ${getCostSummary(dryRunResult).totalCostFormatted}`,
  );
  ...
  const withoutReasoning = await classifyIndicatorsWithOptions(
    sampleIndicators,
    {
      llmConfig: {
        provider: "openai",
  ...
  const tokenIncrease =
    ((summaryWith.totalTokens - summaryWithout.totalTokens) /
  ...
  console.log(
    `Avg cost per indicator: ${largeSummary.avgCostPerIndicatorFormatted}`,
  );
  ...
  for (const count of projections) {
    ...
    console.log(
      `  ${count.toLocaleString().padEnd(10)} indicators: ${
        projection.totalCostFormatted.padEnd(12)
      } (${projection.estimatedTimeFormatted})`,
    );
  }
  ...
  for (const batchSize of batchSizes) {
    ...
    console.log(
      `  Batch size ${
        batchSize.toString().padEnd(2)
      }: ${apiCalls} API calls, ${summary.totalCostFormatted} estimated`,
    );
  }
  ...
  const avgTokensPerIndicator = baseResult.tokenUsage.totalTokens /
    baseResult.summary.total;

  // Calculate costs for each model and quantity
  const { calculateCost } = await import("../src/utils/token_counter.ts");

  console.log(
    "Based on estimated token usage of ~" + Math.round(avgTokensPerIndicator) +
      " tokens per indicator:\n",
  );
  console.log(
    "┌─────────────┬────────────────────┬─────────────┬──────────────────┬─────────────────────┐",
  );
  console.log(
    "│ Indicators  │ Gemini 2.5 Flash   │ GPT-4o      │ Claude 3.5 Sonnet│ Cheapest Option     │",
  );
  console.log(
    "├─────────────┼────────────────────┼─────────────┼──────────────────┼─────────────────────┤",
  );
  ...
  for (const quantity of quantities) {
    ...
    console.log(
      "│ " + quantity.toLocaleString().padEnd(11) + " │ " +
        ...
        (cheapest?.label.includes("gemini")
          ? "Gemini"
          : cheapest?.label.includes("gpt")
          ? "GPT-4o"
          : "Claude").padEnd(19) +
        " │",
    );
