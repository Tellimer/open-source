...
try {
  ...
  for (const indicator of result.enriched) {
    ...
    console.log(`  Classification ID: ${indicator.classification.indicator_id}`);
...
try {
  ...
  for (const indicator of result.enriched) {
    ...
    console.log(`  Matches Classification ID: ${indicator.id === indicator.classification.indicator_id ? '✓' : '✗'}`);
    console.log(`  Type: ${indicator.classification.indicator_type}`);
  }
...
try {
  ...
  console.log(`  Processing Time: ${result.processingTime}ms`);
  console.log(`  Avg Time per Indicator: ${(result.processingTime / result.summary.total).toFixed(0)}ms`);
  ...
  if (result.enriched.length > 0) {
    ...
    for (const indicator of result.enriched.slice(0, 3)) {
      ...
      console.log(`    Type: ${indicator.classification.indicator_type}`);
      console.log(`    Temporal: ${indicator.classification.temporal_aggregation}`);
      console.log(`    Monetary: ${indicator.classification.is_monetary ? 'Yes' : 'No'}`);
      console.log(`    Heat Map: ${indicator.classification.heat_map_orientation}`);
      console.log(`    Confidence: ${(indicator.classification.confidence || 0) * 100}%`);
    }
  }
} catch (error) {
  console.error('Error:', error);
