...
async function main() {
  ...
  console.log("\n💰 Token Usage & Cost:");
  console.log(
    `  Input tokens:          ${formatTokens(result.tokenUsage.inputTokens)}`,
  );
  console.log(
    `  Output tokens:         ${formatTokens(result.tokenUsage.outputTokens)}`,
  );
  console.log(
    `  Total tokens:          ${formatTokens(result.tokenUsage.totalTokens)}`,
  );
  console.log(
    `  Estimated cost:        ${formatCost(result.tokenUsage.estimatedCost)}`,
  );
  ...
  console.log(
    `  Avg cost/indicator:    ${
      formatCost(result.performance.avgCostPerIndicator)
    }`,
  );
  ...
  console.log("\nWithout Reasoning:");
  console.log(
    `  Tokens:  ${formatTokens(withoutReasoning.tokenUsage.totalTokens)}`,
  );
  console.log(
    `  Cost:    ${formatCost(withoutReasoning.tokenUsage.estimatedCost)}`,
  );
  console.log(`  Time:    ${withoutReasoning.processingTime}ms`);

  console.log("\nWith Reasoning:");
  console.log(
    `  Tokens:  ${formatTokens(withReasoning.tokenUsage.totalTokens)}`,
  );
  console.log(
    `  Cost:    ${formatCost(withReasoning.tokenUsage.estimatedCost)}`,
  );
  console.log(`  Time:    ${withReasoning.processingTime}ms`);

  const tokenIncrease = ((withReasoning.tokenUsage.totalTokens -
    withoutReasoning.tokenUsage.totalTokens) /
    withoutReasoning.tokenUsage.totalTokens) * 100;
  const costIncrease = ((withReasoning.tokenUsage.estimatedCost -
  ...
  for (const batchSize of batchSizes) {
    ...
    console.log(
      `  Cost:         ${formatCost(batchResult.tokenUsage.estimatedCost)}`,
    );
    console.log(
      `  Throughput:   ${
        batchResult.performance.throughput.toFixed(2)
      } ind/sec`,
    );
    console.log("");
  }
