...
async function main() {
  ...
  if (!apiKey) {
    console.error(
      "Please set an API key: OPENAI_API_KEY, ANTHROPIC_API_KEY, or GEMINI_API_KEY",
    );
    Deno.exit(1);
  }
  ...
  console.log("=======================");
  console.log(
    `Total estimated cost: $${result.tokenUsage.estimatedCost.toFixed(6)}`,
  );
  console.log(
    `Total tokens: ${result.tokenUsage.totalTokens.toLocaleString()}`,
  );
  ...
  console.log(
    `Avg cost per indicator: ${summary.avgCostPerIndicatorFormatted}`,
  );
  ...
  for (const enriched of result.enriched) {
    console.log(
      `${enriched.name.padEnd(30)} → ${enriched.classification.indicator_type}`,
    );
  }
  console.log("");
}
