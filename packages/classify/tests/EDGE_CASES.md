...
This document explains the edge cases included in the test suite and why they're important for validating LLM classification robustness.

## Overview

Edge cases test boundary conditions, ambiguous classifications, and unusual indicators that don't fit neatly into standard categories. These cases help ensure the classification system handles real-world complexity.
...
Economic indicators don't always fit neatly into categories. Edge cases reflect the messy reality of economic data.

### 2. LLM Robustness
Testing edge cases ensures the LLM can handle ambiguity and make reasonable classifications when indicators are unclear.

### 3. Confidence Calibration
Edge cases should produce lower confidence scores, helping validate the LLM's uncertainty estimation.

### 4. Fallback Behavior
Tests that the system properly uses "other" and "unknown" types when appropriate.

### 5. Boundary Conditions
Validates classification logic at the boundaries between similar types (percentage vs share, ratio vs price, etc.).
...
Edge cases are essential for validating classification robustness. They test the system's ability to handle:
- Ambiguous indicators
- Boundary conditions
- Unconventional metrics
- Real-world complexity

By including comprehensive edge cases in the test suite, we ensure the classification system is production-ready and can handle the full spectrum of economic indicators.
